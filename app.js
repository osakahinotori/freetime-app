...
(function () {
  ...
  function getExcludeRanges() {
    return [...document.querySelectorAll("#excludeList .exclude-row")].map((r) => ({
  ...
  function setupWorkDefaults() {
    ...
    if (saved && saved.length) saved.forEach((w) => addWorkRow(w.from, w.to));
    else addWorkRow("09:00", "17:00");
  ...
  function addWorkRow(from = "09:00", to = "17:00") {
    const row = document.createElement("div");
    row.className = "exclude-row";
    row.innerHTML = `
      ...
      <button class="del" title="削除">×</button>`;
    row.querySelector(".del").onclick = () => { row.remove(); saveWork(); };
    row.querySelectorAll("input").forEach((i) => (i.onchange = saveWork));
    $("workList").appendChild(row);
  }
  function getWorkBands() {
    return [...document.querySelectorAll("#workList .exclude-row")].map((r) => ({
