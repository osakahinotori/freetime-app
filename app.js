...
(function () {
  ...
  function setupWorkDefaults() {
    ...
    if (saved && saved.length) saved.forEach((w) => addWorkRow(w.from, w.to, w.days || [0, 1, 2, 3, 4, 5, 6]));
    else addWorkRow("09:00", "17:00", [1, 2, 3, 4, 5]); // 既定は平日
  ...
  function addWorkRow(from = "09:00", to = "17:00", days = [1, 2, 3, 4, 5]) {
    ...
    wrap.querySelectorAll(".dow").forEach((b) => (b.onclick = () => { b.classList.toggle("on"); saveWork(); }));
